<template>
  <div class="container">
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(assetInfo, index) in assetsInfo" :key="index">
          <td>{{ assetInfo.symbol }}</td>
          <td>{{ assetInfo.quantity }}</td>
          <td>{{ assetInfo.price ? assetInfo.price.toFixed(2) : '-' }}</td>
          <td>{{ assetInfo.price ? (assetInfo.quantity * assetInfo.price).toFixed(2) : '-' }}</td>
          <td>
            <div class="action-buttons">
              <button class="edit-button" @click="openEditModal(assetInfo)">Edit</button>
              <button class="delete-button" @click="deleteAsset(assetInfo.id)">Delete</button>
            </div>
          </td>        
        </tr>
      </tbody>
...
export default {
  ...
  methods: {
    renderDoughnutChart() {
      ...
      const labels = this.assetsInfo.map(assetInfo => assetInfo.symbol);
      const data = this.assetsInfo.map(assetInfo => assetInfo.quantity * assetInfo.price);
    ...
    listAssets() {
      ...
      client.listAssets(request, {}, (err, response) => {
        if (err) {
          ...
          this.assetsInfo = response.toObject().assetsInfoList;
          this.renderDoughnutChart();
        }
      });
    }
