...
func (s *server) UpdateAsset(ctx context.Context, req *asset.UpdateAssetRequest) (*asset.Asset, error) {
  ...
  if req.AssetType == asset.AssetType_CRYPTOCURRENCY {
      // Cryptocurrency pair
      pair, err := crypto.Get(req.Symbol)
      if err != nil {
          return nil, err
      }
      latestPrice = pair.RegularMarketPrice
  } else if req.AssetType == asset.AssetType_STOCK {
      // Stock symbol
      ...
      return nil, fmt.Errorf("unsupported asset type: %v", req.AssetType)
  }
